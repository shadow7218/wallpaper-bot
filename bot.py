...
async def analyze_image_with_gemini(image_path: str) -> dict:
    ...
    try:
        ...
        payload = {
            "contents": [{"parts": [
                {"text": "Analyze this anime wallpaper. Identify main characters and category. Return only lowercase hashtags separated by spaces. Example: #naruto #animewallpaper. If you cannot identify a specific character, just return the category hashtag like #animewallpaper or #anime."},
...
async def do_post(bot):
    ...
    # Collect all hashtags from all wallpapers for the album caption
    all_tags = []
    ...
    # Build the caption with all hashtags
    ...
    try:
        ...
        for i, wp in enumerate(wallpapers):
            with open(wp["path"], "rb") as f:
                photo_bytes = f.read()
            # Only the first photo in the group gets the caption
            if i == 0:
    ...
    # --- STEP 2: Send all files as a media group (documents) without hashtags ---
    try:
        doc_media_group = []
        for wp in wallpapers:
            with open(wp["path"], "rb") as f:
                doc_bytes = f.read()
            doc_media_group.append(InputMediaDocument(media=doc_bytes))
